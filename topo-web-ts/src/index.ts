...
let require7 = require('./custom-dep/layui/css/modules/layer/default/layer.css');
let require8 = require('./custom-dep/layui/css/modules/laydate/default/laydate.css');
let require9 = require('./custom-dep/layui/css/modules/code.css');

let require5 = require('./css/topo-main.css');
let require2 = require('./css/displayControl.css');
let require1 = require('./custom-dep/scant/scant.css');
let require3 = require('./css/icon.css');
let require4 = require('./css/layui-plus.css');
let require6 = require('./css/vis-extend.css');
...
$(function () {
    ...
    function topoInit() {
        var option = {
            manipulation: {
                ...
                addNode: false,
                addEdge: function (param: any, callBack: any) {
                    ...
                    var isExist = false;
                    var existId = '';
                    $.each(ParamHolder.data.edges.get(), function (index: any, element: any) {
                    ...
                    isExist && layer.confirm('两节点已存在连接线，是否要删除之前的连接线（包括连接线信息）', function (index: any) {
                        callBack(param);
                        ParamHolder.data.edges.remove(existId);
                        ParamHolder.network.addEdgeMode();
                        layer.close(index);
    ...
    function topoDataListener() {
        ParamHolder.data.nodes.on('add', function (event: any, properties: any, senderId: any) {
            console.log(event);
            console.log(properties);
            console.log(senderId);
            $.each(properties.items, function (index: any, id: any) {
        ...
        ParamHolder.data.nodes.on('remove', function (event: any, properties: any, senderId: any) {
        ...
        ParamHolder.data.edges.on('add', function (event: any, properties: any, senderId: any) {
            $.each(properties.items, function (index: any, id: any) {
        ...
        ParamHolder.data.edges.on('remove', function (event: any, properties: any, senderId: any) {
        ...
        ParamHolder.info.nodes.on('update', function (event: any, properties: any, senderId: any) {
            //更新topo图上的表现形式
            $.each(properties.items, function (index: any, id: any) {
        ...
        ParamHolder.info.edges.on('update', function (event: any, properties: any, senderId: any) {
            //TODO 更新topo图上的连接线形式
            $.each(properties.items, function (index: any, id: any) {
