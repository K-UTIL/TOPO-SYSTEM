...
export class Utils {
    ...
    public static getWithDefault(content: string | number, defaults: string | number): string | number {
        return content ? content : defaults;
    }

    public static getParam(key: string, defaults: string): string {
        if (this.getWithDefault(key, "") === "") return defaults;
        var matchArray = window.location.search.match(new RegExp('(\\?|\&)' + key + '=([^\&]+)'));
        return matchArray ? matchArray[2] : defaults;
    }

    public static parseServer(dataSets: any, infoSets: any, stNumber: string): any {
        var me = this;
    ...
    public static parseFromServer(respData: any,infoSets:any,dataSets:any) {
